...
<?php $idUser = $_GET ['idUsuario']; ?>
<?php include('includes/getposts.php'); ?>
<?php


$result= $mysqli->query($consultaUser);
...
<div class="row">
    <?php if(empty($checkPosts)){ ?>
        <div class="col-3"></div>
        <div class="col-6">
            <h2>ESTE USUARIO NO TIENE PUBLICACIONES HECHAS TODAVIA </h2>
        </div>
        <?php }else { ?>
        <div class="col-4"></div>
        <div class="col-6">
            <h1 class='generalTitle'>POSTS DE:  <?php echo ($checkPosts['nombreUsuario']); ?>    <?php echo ($checkPosts['apellidoUsuario']); ?>  </h1>
    </div>
</div>
<div class="row">  </div>
<div class="row">
